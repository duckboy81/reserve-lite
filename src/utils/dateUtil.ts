...
export const generateTimeline = (startStr: string, endStr: string, config: Config): TimelineRowData[] => {
  ...
  while (keepGoing && limit < SAFETY_BREAK) {
    ...
    if (reportDateStr < startStr) { iterator = new Date(iterator.getTime() + 60 * 60 * 1000); continue; }
    if (reportDateStr === startStr && reportTz.hour < 10) { iterator = new Date(iterator.getTime() + 60 * 60 * 1000); continue; }
    ...
    if (callDateStr > endStr) { keepGoing = false; break; }
    if (callDateStr === endStr && (callTz.hour > 6 || (callTz.hour === 6 && callTz.minute > 0))) { keepGoing = false; break; }

    const callHomeTz = getTimeInTz(callTime, config.homeTz);
    const callDateDisplay = new Intl.DateTimeFormat('en-US', { timeZone: config.homeTz, month: 'short', day: 'numeric' }).format(callTime);
