        ...
        {importState.stage === "preview" ? (
          <div className="space-y-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div className="bg-yellow-50 text-yellow-800 text-xs p-2 rounded flex gap-2 items-start">
              ...
              <p>
                Importing will <strong>permanently replace</strong> all current data. This cannot be undone.
              </p>
            </div>
            <div className="flex gap-2 justify-end pt-2">
              <button onClick={() => setImportState({ stage: "idle" })} className="px-3 py-1 text-gray-500 text-sm">
                Cancel
              </button>
              <button onClick={executeImport} className="px-3 py-1 bg-indigo-600 text-white rounded text-sm font-bold">
                Confirm & Import
              </button>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  value={localConfig.homeBase}
                  onChange={(e) => setLocalConfig({ ...localConfig, homeBase: e.target.value.toUpperCase() })}
                />
              </div>
              <div>
                ...
                <select
                  ...
                  value={localConfig.homeTz}
                  onChange={(e) => setLocalConfig({ ...localConfig, homeTz: e.target.value })}
                >
                  {TIMEZONES.map((tz) => (
              ...
              <div>
                ...
                <input
                  className="w-full border p-2 rounded"
                  value={localConfig.reserveBase}
                  onChange={(e) => setLocalConfig({ ...localConfig, reserveBase: e.target.value.toUpperCase() })}
                />
              </div>
              <div>
                ...
                <select
                  className="w-full border p-2 rounded bg-white"
                  value={localConfig.reserveTz}
                  onChange={(e) => setLocalConfig({ ...localConfig, reserveTz: e.target.value })}
                >
                  {TIMEZONES.map((tz) => (
                    <option key={tz.value} value={tz.value}>
                      {tz.label}
            ...
            <div className="border-t pt-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 border border-gray-300 rounded p-2 text-sm hover:bg-gray-50 text-gray-700"
                >
