...
const FlightForm: React.FC<FlightFormProps> = ({
    ...
    useEffect(() => {
        ...
        if (initialSegment) {
            ...
            const d = baseDate || new Date().toISOString().split('T')[0];
            ...
            setFormData({
                ...
                // We might need to add gate to FlightSegment or just lose it on save.
                // For now, we will track it in UI but if FlightSegment doesn't have it, it won't persist unless we add it to type.
                // User's redesign shows Gate inputs.
                // I will check types.ts again.
                // FlightSegment: { flight, dep, arr, status, ground, depAirport, arrAirport, ... }
                // No gate. I will stick to what exists or maybe put it in 'status' for now?
                // Actually, I'll just leave it as UI state. If types don't support it, it won't save.
                arrGate: "",
                dep: depIso,
                arr: arrIso,
            });
            ...
            setSearchParams({ from: initialSegment.depAirport || defOrg, to: initialSegment.arrAirport || defDst, date: baseDate });
    ...
    const selectSearchResult = (res: SearchResult) => {
        ...
        if (type === "inbound" && leg.arrivalAirportCode !== hubAirport) {
            // If we are flying inbound to hub, but land somewhere else, assume ground logic?
            // Actually, standard logic: check destination.
            ...
            <div className="px-6 py-4 border-b border-slate-100 bg-white z-10">
                <div className="flex p-1 bg-slate-100 rounded-lg">
                    <button
                        ...
                        className={`flex-1 py-1.5 text-sm font-bold rounded-md transition-all flex justify-center items-center gap-2 ${mode === 'search' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
            ...
            <div className="flex-1 overflow-y-auto p-6 min-h-0">
                {mode === "search" ? (
                            ...
                            <div className="mt-0">
                                <div className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-lg p-3 mb-4">
                                    <div className="flex items-center gap-2 text-sm">
                                        <span className="font-bold text-slate-800">{searchParams.from} <span className="text-slate-400">→</span> {searchParams.to}</span>
                                        <span className="text-slate-300">|</span>
                                        <span className="text-slate-500">{new Date(searchParams.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                                    </div>
                                    <button onClick={() => setSearchResults(null)} className="text-xs font-bold text-indigo-600 hover:text-indigo-800">Edit</button>
                                </div>

                                <h4 className="text-xs font-bold text-slate-400 uppercase mb-3 text-left">Results found ({searchResults.length})</h4>
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className="bg-indigo-50 text-indigo-600 text-[10px] font-bold w-8 h-8 rounded flex items-center justify-center">{leg.carrierCodeIATA}</div>
                                                    <div>
                                                        <div className="font-bold text-slate-800 text-sm relative">
                                                            {dep.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })} - {arr.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })}
                                                            {isNextDay && <span className="text-[0.6em] text-indigo-500 font-bold ml-0.5 -mt-1.5 absolute">+1</span>}
                                                        </div>
                                                        <div className="text-xs text-slate-400 flex items-center gap-1.5">
                                                            <span>{leg.carrierCodeIATA} {leg.aircraftIdentification.flightNumber}</span>
                                                            <span className="w-1 h-1 rounded-full bg-slate-300"></span>
                                                            <span>{leg.departure.gate || "--"}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="bg-slate-50 text-slate-400 px-2 py-1 rounded text-xs font-bold group-hover:bg-indigo-600 group-hover:text-white transition-colors">Select</div>
                                            </button>
                                        );
                                    })}
                    ...
                    <div className="space-y-4">
                        {/* DETAILS FORM */}
                        <div>
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Flight Number</label>
                            <input
                                type="text"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Dep Gate</label>
                                <input
                                    type="text"
                                    className="w-full border rounded-lg p-2 text-sm font-bold uppercase focus:ring-2 focus:ring-indigo-600 outline-none border-slate-200"
                                    placeholder="--"
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Arr Gate</label>
                                <input
                                    type="text"
                                    className="w-full border rounded-lg p-2 text-sm font-bold uppercase focus:ring-2 focus:ring-indigo-600 outline-none border-slate-200"
                                    placeholder="--"
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Departure</label>
                                ...
                                <div>
                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Arrival</label>
                                    <div className="relative group text-left">
                                        <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                                            <Calendar className="w-4 h-4 text-slate-400 group-hover:text-indigo-500 transition-colors" />
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            ...
                                            <div className="flex gap-2">
                                                <div className="w-full">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Hrs</label>
                                                    <input
                                                        type="number"
                                                        className="w-full border rounded-lg p-2 text-sm font-medium bg-white focus:ring-2 focus:ring-indigo-600 outline-none border-indigo-200 text-slate-700"
                                                ...
                                                <div className="w-full">
                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Min</label>
                                                    <input
                                                        type="number"
                                                        className="w-full border rounded-lg p-2 text-sm font-medium bg-white focus:ring-2 focus:ring-indigo-600 outline-none border-indigo-200 text-slate-700"
