...
export function useScheduleManager() {
    ...
    const modifyOptions = (airport: string, rowId: string, action: (options: Option[]) => void) => {
        ...
        if (!isEditMode) {
            setIsEditMode(true);
            // If we weren't in edit mode, we need to set staging data now
            // Re-cloning effectively starts the session
        }

        const newData = JSON.parse(JSON.stringify(currentData));
        // Ensure airport array exists
        ...
        if (row) {
            ...
            // If row doesn't exist but we want to modify it (e.g. adding), we might need to create it.
            // But modifyOptions usually assumes row exists or we are finding it.
            // App.tsx handleAddOptionLogic was: 
            // saveOptionToStaging handles creation. modifyOptions handles existing.
            // Let's rely on saveOptionToStaging for creation if needed, or handle it here?
            // In App.tsx modifyOptions was strictly for 'row exists'.
        }
    };

    const saveOptionToStaging = (airport: string, rowId: string, index: number | null, option: Option, dateContext: string) => {
